...
class linkedlist:
    ...
    def isSame(self, other_list):
        ...
        # Nếu một danh sách dài hơn danh sách kia
...
print("deleted from head:")
mylist.traverse()
#
print("deleted from tail:")
mylist.deleteFromTail()
mylist.traverse()
# Xóa phần tử sau phần tử có giá trị là 2
node_2 = mylist.head.next.next
deleted_node = mylist.deleteAfter(node_2)
if deleted_node:
    print("Phần tử đã bị xóa:", deleted_node.data)
#
print("deleted X:")
mylist.delX(2)
mylist.traverse()
# Search for the node with data value 3
node = mylist.search(7)
if node:
    # Delete the node after the node with data value 3
    deleted_node = mylist.deleteAfter(node)
    if deleted_node:
        print("Deleted value:", deleted_node.data)
    else:
        print("No node found after the given node.")
else:
...
linked_list.addtoHead("Charlie")
linked_list.addAfter("Bob", "Dave")

print("Singly Linked List Q2:") 
